...
COEFF generate_free_coeff_from_string(string cc, const unsigned short ssector) {
    ...
    if ((ssector == 0) && (!common::receive_from_child)) {
        cc = calc(cc, 0);
    } else {
        calc_wrapper(cc, 0);
    }
    ...
    while (fgets(load_string, sizeof(load_string), file)) {
        ...
        if (load_string[1] == '\n' || load_string[0] == '\n') {
            ...
            while (str.find('G') != string::npos) {   // here we build it just with strings
                ...
                sprintf(buf, "%s|%lu", current_coeff.c_str(), static_cast<unsigned long int>(common::prime)); // or it can become big!
                if ((sector == 0) && (!common::receive_from_child)) {
                    current_coeff = calc(buf, 0);
                } else {
                    current_coeff = buf;
                    calc_wrapper(current_coeff, 0);
                }
...
#ifdef __APPLE__
    fermat = common::FIRE_folder + "../extra/ferm6/fer64";
#else
    fermat = common::FIRE_folder + "../extra/ferl6/fer64";
#endif
